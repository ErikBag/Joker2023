package me.markoutte.joker.parse.my_parse
...
fun main(args: Array<String>) {
    ...
    val seeds = mutableMapOf(
        -1 to """<!DOCTYPE html><html><head></head><body></body></html>""".asByteArray(b.size),
        0 to """<html><body><h1>Test</h1></body></html>""".asByteArray(b.size),
        1 to """<html><p>Invalid HTML <div><span></body></html>""".asByteArray(b.size)
    )
...
fun Random.mutateSwapBytes(buffer: ByteArray): ByteArray {
    if (buffer.size < 2) return buffer.copyOf()  // No change if buffer has less than 2 bytes
    val (pos1, pos2) = generateTwoDistinctRandomIndices(buffer.size)
    buffer.swap(pos1, pos2)  // Swap bytes at the selected positions
    return buffer.copyOf()  // Return a copy to avoid modifying the original buffer
}

fun Random.mutateRandomizeBytes(buffer: ByteArray): ByteArray {
    val pos = nextInt(buffer.size)
    buffer[pos] = nextInt(256).toByte()  // Randomize a single byte
    return buffer.copyOf()  // Return a copy to avoid modifying the original buffer
}

// Helper function to generate two distinct random indices
fun Random.generateTwoDistinctRandomIndices(size: Int): Pair<Int, Int> {
    val pos1 = nextInt(size)
    var pos2 = nextInt(size)
    // Ensure pos1 and pos2 are distinct
    while (pos1 == pos2) {
        pos2 = nextInt(size)
    }
...
// Helper function to swap two elements in a ByteArray
fun ByteArray.swap(index1: Int, index2: Int) {
